...
### Comprehensive Deployment with ArgoCD Integration

```yaml
name: Deploy with ArgoCD and Record in DX
  ...
  deploy:
    ...
    steps:
      ...
      - name: Deploy with ArgoCD
        id: deploy
        run: |
          # ArgoCD deployment logic here
          echo "deployment_url=https://argocd.example.com/applications/my-app" >> $GITHUB_OUTPUT
          echo "Deploying to ${{ github.event.inputs.environment }}..."
      
      - name: Record successful deployment in DX
        if: success()
        uses: Luscii/gha-dx-create-deployment@v1
        with:
          dx_instance: 'luscii'
          ...
          success: 'true'
          reference_id: 'deploy-${{ github.run_number }}'
          source_url: ${{ steps.deploy.outputs.deployment_url }}
          source_name: 'ArgoCD'
          metadata: |
            {
      ...
      - name: Record failed deployment in DX
        if: failure()
        uses: Luscii/gha-dx-create-deployment@v1
        with:
          dx_instance: 'luscii'
